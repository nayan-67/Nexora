...
class Product extends Controller
{
    ...
    {
        $product = Products::with(['category', 'sub_category', 'variants'])
            ->where('id', $id)
            ->where('is_delete', '0')
            ->first();
        ...
        if ($product->type == 2 && $product->variants->count()) {
            $prices = $product->variants->map(function ($variant) {
                return $variant->is_sale ? ($variant->sale_price ?? $variant->price) : $variant->price;
            })->filter()->values();

            if ($prices->count()) {
                $min = $prices->min();
                $max = $prices->max();
                $variantPriceRange = $min === $max ? '₹ ' . $min : '₹ ' . $min . ' - ₹ ' . $max;
            }
        }
        ...
        return response()->json([
            ...
            'gallery_images' => $product->gallery_images ?? $product->gallery_image,
            'stock' => $product->stock,
            ...
            'variant_price_range' => $variantPriceRange,
        ]);
    }
    ...
    public function storeVariant(Request $request)
    {
        ...
        try {
            ...
            $product->sku = substr($variantsData[0]['sku'], 0, 6);
            $product->save();
